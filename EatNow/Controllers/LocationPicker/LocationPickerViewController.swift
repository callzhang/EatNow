...
public class LocationPickerViewController: UIViewController,LocationCurrentViewDelegate {
	...
	public var mapType: MKMapType = .Satellite {
		didSet {
			if isViewLoaded() {
...
//		mapView.addGestureRecognizer(UILongPressGestureRecognizer(target: self, action: "addLocation:"))
        ...
        //back button
        let backBt = UIBarButtonItem(title: "返回", style: .Done, target: self, action: "back")
     ...
     func back(){
    NSNotificationCenter.defaultCenter().postNotificationName("CancelChangeLocationUpdated", object: nil, userInfo: nil)
