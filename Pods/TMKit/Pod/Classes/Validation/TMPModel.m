...
//meta properties
NSString *const TMPModelValidatorType = @"-validator";

//all known properties for built-in validators/filters
NSString *const TMPModelValidatorAttributes = @"attributes";
NSString *const TMPModelValidatorSkipOnError = @"skipOnError";
NSString *const TMPModelValidatorSkipOnEmpty = @"skipOnEmpty";
NSString *const TMPModelValidatorMessage = @"message";
NSString *const TMPModelValidatorIsEmpty = @"isEmpty";
NSString *const TMPModelValidatorAction = @"action";
NSString *const TMPModelValidatorTrueValue = @"trueValue";
...
NSString *const TMPModelValidatorFilter = @"filter";
NSString *const TMPModelValidatorSkipOnArray = @"skipOnArray";
NSString *const TMPModelValidatorTooBig = @"tooBig";
NSString *const TMPModelValidatorTooSmall = @"tooSmall";
NSString *const TMPModelValidatorMin = @"min";
NSString *const TMPModelValidatorMax = @"max";
NSString *const TMPModelValidatorRange = @"range";
NSString *const TMPModelValidatorAllowArray = @"allowArray";
NSString *const TMPModelValidatorNot = @"not";
NSString *const TMPModelValidatorPattern = @"pattern";
NSString *const TMPModelValidatorRequiredValue = @"requiredValue";
NSString *const TMPModelValidatorLength = @"length";
NSString *const TMPModelValidatorTooShort = @"tooShort";
NSString *const TMPModelValidatorTooLong = @"tooLong";
NSString *const TMPModelValidatorNotEqual = @"notEqual";
NSString *const TMPModelValidatorValidSchemes = @"validSchemes";
NSString *const TMPModelValidatorDefaultScheme = @"defaultScheme";
NSString *const TMPModelValidatorSet = @"set";
NSString *const TMPModelValidatorOn = @"on";
NSString *const TMPModelValidatorExcept = @"except";
NSString *const TMPModelValidatorWhen = @"when";
#pragma clang diagnostic pop

@interface TMPModelWrapper () <TMPModelValidation>
@end

@implementation TMPModelWrapper
@dynamic scenario;
@dynamic validators;
@dynamic attributes;
...
#pragma mark - TMPModelValidation protocol default implementation
-(NSArray *)rules {
...
#pragma mark - TMPModel protocol default implementation
-(NSDictionary *)scenarioList {
	...
	for(TMPModelValidator *validator in [self getValidators]) {
...
-(BOOL)tm_validate {
	return [self tm_validate:nil clearErrors:YES];
}

-(BOOL)tm_validate:(NSArray *)attributes{
	return [self tm_validate:attributes clearErrors:YES];
}

-(BOOL)tm_validate:(NSArray *)attributes clearErrors:(BOOL)clearErrors {
	if(clearErrors)
	...
	if([self shouldValidateModel]) {
		...
		if(scenarios[scenario] == nil)
			@throw [NSException exceptionWithName:@"TMPModel" reason:[NSString stringWithFormat:@"Unknown scenario: %@", scenario] userInfo:nil];
...
-(NSMutableArray *)createValidators {
	...
	for(id rule in rules) {
		if ([rule isKindOfClass:[TMPModelValidator class]]) {
			validator = rule;
		} else {
			...
			if ([rule isKindOfClass:[NSArray class]] && ([(NSArray *)rule count] > 1)) {
				...
				attributes = rule[0];
				params = nil;
			} else if ([rule isKindOfClass:[NSDictionary class]] && rule[TMPModelValidatorAttributes] && rule[TMPModelValidatorType]) {
				type = rule[TMPModelValidatorType];
				attributes = rule[TMPModelValidatorAttributes];
				params = [(NSDictionary *)rule mutableCopy];
				[params removeObjectForKey:TMPModelValidatorAttributes];
				[params removeObjectForKey:TMPModelValidatorType];
			} else {
				@throw [NSException exceptionWithName:@"TMPModel" reason:@"Invalid validation rule: a rule must specify both attribute names and validator type." userInfo:nil];
			}
			...
			if(!([attributes isKindOfClass:[NSArray class]]))
				@throw [NSException exceptionWithName:@"TMPModel" reason:@"Only NSArray or comma-separated NSString are supported as list of attributes." userInfo:nil];

			validator = [TMPModelValidator createValidator:type
													model:self
		...
		if(!(validator))
			@throw [NSException exceptionWithName:@"TMPModel" reason:[NSString stringWithFormat:@"Can't create a validator for rule: %@", rule] userInfo:nil];
...
-(BOOL)isAttributeRequired:(NSString *)attribute {
	...
	for(TMPModelValidator *validator in [self getActiveValidators:attribute]) {
		if([validator isKindOfClass:[TMPModelRequiredValidator class]] && validator.when == nil) {
			return YES;
		}
	}
...
-(void)validateUpdates:(NSArray *)attributes except:(NSArray *)except {
	@synchronized (self) {
		TMPModelObserver *observer = [self getUpdatesObserver];

		if(observer == nil) {
			observer = [[TMPModelObserver alloc] initWithModel:self];
...
#pragma mark - TMPModel private methods
...
-(TMPModelObserver *)getUpdatesObserver {
	return (TMPModelObserver *)objc_getAssociatedObject(self, @selector(getUpdatesObserver));
}
@end
