...
#import "TMPModelEmailValidator.h"
#import "TMPModelUrlValidator.h"

@implementation TMPModelEmailValidator
-(instancetype)init {
	if((self = [super init])) {
		_checkDNS = NO;
		_enableIDN = YES;
...
-(NSString *)message {
	return ([super message] ? [super message] : @"{attribute} is not valid.");
}
...
-(NSError *)validateValue:(id)value {
	...
	if(value && [value isKindOfClass:[NSString class]] && (parts = [self parseEmail:value])) {
		...
		if (_enableIDN)
			value = [TMPModelUrlValidator encodeIDN:value];
		...
		if(_checkDNS) {
			...
			if(error) {
				return [NSError errorWithDomain:FXFormValidatorErrorDomain
										   code:0
									   userInfo:@{
											   NSLocalizedDescriptionKey: @"{attribute} cannot be resolved.",
											   @"Reason": error,
									   }];
			}
		}
