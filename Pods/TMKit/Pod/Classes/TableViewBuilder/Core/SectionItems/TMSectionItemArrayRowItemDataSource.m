...
- (void)addRowItem:(TMRowItem *)rowItems {
...
- (void)removeAllRowItems {
    NSArray *rowItems = [self.rowItems copy];
    [self.rowItems removeAllObjects];
    [rowItems enumerateObjectsUsingBlock:^(TMRowItem *rowItem, NSUInteger idx, BOOL *stop) {
        [self didRemoveRowItem:rowItem];
    }];
}
