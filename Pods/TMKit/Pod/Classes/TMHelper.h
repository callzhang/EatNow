...
#define TMIMPLStringKey(x) NSString *const x = @STRINGIFY(x)
#define TMDefineStringKey(x) extern NSString *const x

#define TMSuppressPerformSelectorWarning(PerformCall) \
do { \
...
void tm_runOnMainQueueWithoutDeadlocking(void (^block)(void));